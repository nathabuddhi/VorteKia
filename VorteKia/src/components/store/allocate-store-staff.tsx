...
import {
    allocateStaffPromise,
    clearStaffAllocation,
...
import {
    getAllStoreStaff,
    getAssignedStoreStaffByStore,
} from "@/controllers/staff-controller";
import { Separator } from "@/components/ui/separator";
import { Checkbox } from "@/components/ui/checkbox";

export default function AllocateStoreStaff({ storeId }: { storeId: string }) {
    ...
    const fetchAllocatedStaff = async () => {
        const response = await getAssignedStoreStaffByStore(storeId);
    ...
    const fetchAllStaff = async () => {
        const response = await getAllStoreStaff();
    ...
    const handleAllocateStaffSubmit = async () => {
        setOpen(false);
        toast.promise(clearStaffAllocation(storeId), {
            loading: "Clearing store allocation...",
            success: (response) => {
                if (response.success) {
                    return "Cleared store staff allocation. Allocating staff now...";
                } else {
        ...
        for (const staff_id of allocatedStaff) {
            try {
                toast.promise(allocateStaffPromise(storeId, staff_id), {
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ...
                        allocated staff will overstore the previous allocation.
                    </DialogDescription>
                </DialogHeader>
